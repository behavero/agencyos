...
export async function GET(request: Request) {
  ...
  if (providerError) {
    ...
    return NextResponse.redirect(new URL(`${baseUrl}/dashboard/creator-management?${search.toString()}`));
  }

  if (!code || !state || !storedState || !verifier || state !== storedState) {
    console.error("[OAuth Callback] State mismatch or missing data");
    return NextResponse.redirect(new URL(`${baseUrl}/dashboard/creator-management?error=oauth_state_mismatch`));
  }
  ...
  try {
    ...
    if (!userRes.ok) {
      ...
      return NextResponse.redirect(new URL(`${baseUrl}/dashboard/creator-management?error=user_fetch_failed`));
    }
    ...
    if (!profile?.agency_id) {
      return NextResponse.redirect(new URL(`${baseUrl}/dashboard/creator-management?error=no_agency`));
    }
    ...
    return NextResponse.redirect(new URL(`${baseUrl}/dashboard/creator-management?success=connected`));

  } catch (e: any) {
    console.error("[OAuth Callback] Error:", e);
    return NextResponse.redirect(new URL(`${baseUrl}/dashboard/creator-management?error=oauth_token_exchange_failed&details=${encodeURIComponent(e.message)}`));
  }
}
